...
export class CalendarWidgetComponent implements OnDestroy {
  ...
  constructor(private el: ElementRef, private appointmentsService: AppointmentsService) {
  ...
  render() {
    ...
    this.calendar = this.$calendarRef.fullCalendar({
        ...
        drop: (date, event, ui) => { // this function is called when something is dropped
          ...
          let originalAppointmentObject = ui.helper.data('eventObject');

          // we need to copy it, so that multiple events don't have a reference to the same object
          let copiedAppointmentObject = $.extend({}, originalAppointmentObject);

          // assign it the date that was reported
          copiedAppointmentObject.start = date;

          // render the event on the calendar
          // the last `true` argument determines if the event "sticks" (http://arshaw.com/fullcalendar/docs/event_rendering/renderEvent/)
          this.$calendarRef.fullCalendar('renderEvent', copiedAppointmentObject, true);

          this.appointmentsService.addAppointment(copiedAppointmentObject)
        ...
        events: (start, end, timezone, callback) => {
          callback(this.appointmentsService.store.appointments)
        },

        appointmentRender: (appointment, element, icon) => {
          if (appointment.description != "") {
            element.find('.fc-event-title').append("<br/><span class='ultra-light'>" + appointment.description + "</span>");
          }
          if (appointment.icon != "") {
            element.find('.fc-event-title').append("<i class='air air-top-right fa " +appointment.icon + " '></i>");
          }
        }
      }
    );
